...
void Move_or_Push(int Dir, Point& Load, Point* Box, int Boxes) {
        int BNum;
        int tmp;
        if (Dir == 1 && Is_A_Box(Box, Load.x, Load.y - 1, Boxes, BNum) &&
!Is_A_Box(Box, Load.x, Load.y - 2, Boxes, tmp)) {
                Push(Dir, Load, Box[BNum], BNum);
        }
        else if (Dir == 2 && Is_A_Box(Box, Load.x - 1, Load.y, Boxes, BNum)
&& !Is_A_Box(Box, Load.x - 2, Load.y, Boxes, tmp)) {
                Push(Dir, Load, Box[BNum], BNum);
        }
        else if (Dir == 3 && Is_A_Box(Box, Load.x, Load.y + 1, Boxes, BNum)
&& !Is_A_Box(Box, Load.x, Load.y + 2, Boxes, tmp)) {
                Push(Dir, Load, Box[BNum], BNum);
        }
        else if (Dir == 4 && Is_A_Box(Box, Load.x + 1, Load.y, Boxes, BNum)
&& !Is_A_Box(Box, Load.x + 2, Load.y, Boxes, tmp)) {
                Push(Dir, Load, Box[BNum], BNum);
        }
